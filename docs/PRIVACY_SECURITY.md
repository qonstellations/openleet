...
OpenLeet has no backend, analytics, telemetry, error reporting, accounts, subscriptions, or history. It does not intentionally collect credentials, code, problem text, solution references, or analysis. References and analysis results live only in the current page's memory and are discarded on navigation/reload. The selected provider may process or retain the problem, code, and reference excerpts according to its own policy.
